...
while not os.path.isdir(os.path.join(_log_root, 'log')):
    ...
    if _p == _log_root:
        break
    _log_root = _p
sys.stdout = _TeeOutput(os.path.join(_log_root, 'log', 'pronostici-del-giorno.txt'))
sys.stderr = sys.stdout
print(f"{'='*50}")
print(f"AVVIO PRONOSTICI: {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}")
print(f"{'='*50}\n")
...
# Soglie decisione
THRESHOLD_INCLUDE = 56.5    # Sotto 56.5 = SCARTA (ottimizzato backtest 07/04/2026)
...
THRESHOLD_BOMBA = 65
THRESHOLD_GOL = 66.1  # Soglia specifica per Over/Under (separata da THRESHOLD_INCLUDE) — #1 backtest range A-S
...
def score_quote(match_data):
    ...
    # 1.55-2.20 = range ideale, picco a 1.639 (ottimizzato backtest 07/04/2026)
        ...
        score = 100 - (distance_from_ideal * 98.53)  # 100 a 1.639, scende ai bordi
...
def score_affidabilita(match_data):
    ...
    # Punteggio base dall'affidabilità numerica della favorita (ottimizzato backtest 07/04/2026)
    ...
    if aff_sfidante >= 7:
        ...
        score -= 47.12  # Sfidante imprevedibile
...
def score_dna(match_data):
    ...
    # Calcola "potenza" complessiva DNA per squadra (ottimizzato backtest 07/04/2026)
    home_power = (
...
def score_motivazioni(match_data, home_team_doc, away_team_doc):
    ...
    # Favorita motivata (5-15 → 0-100) (ottimizzato backtest 07/04/2026)
...
def calcola_stake_kelly(quota, probabilita_stimata, tipo='GOL'):
    ...
    Mantenuta come wrapper sottile per compatibilita` con eventuali chiamate
    residue. Usa il nuovo Kelly unificato con source='A' (Sistema A).
    """
    res = kelly_unified(db, probabilita_stimata, quota,
                        source='A', mercato=tipo, kelly_fraction=0.25)
...
def run_daily_predictions(target_date=None, match_time_filter=None):
    ...
    Args:
        ...
        match_time_filter: lista di orari (es. ['15:00', '15:30']) per filtrare
                          solo i match di quel gruppo orario. Se None, processa tutti.
    """
    ...
    for match in matches:
        ...
        if match.get('_source') == 'cup':
            ...
            if segno_result is None:
                # Quote 1X2 mancanti: segno neutro, GOL può comunque essere analizzato
                segno_result = {
        ...
        for p in decision['pronostici']:
            ...
            kelly = kelly_unified(db, prob_result['probabilita_stimata'],
                                  quota_prono, source='A', mercato=tipo,
                                  kelly_fraction=0.25)

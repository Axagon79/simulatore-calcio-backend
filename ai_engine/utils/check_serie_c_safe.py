...
for i, girone in enumerate(gironi):
    ...
    print(f"📊 Squadre: {team_count}")
    
    # STRENGTH SCORES
    strength_stats = list(teams.aggregate([
    ...
    print(f"👥 Giocatori TM: {avail_count}")
    
    # TOP 5
    top_teams = list(teams.find({"league": league_name}).sort("stats.strengthScore09", -1).limit(5))
    print("🏅 TOP 5:")
    for team in top_teams:
        name = team.get("name", "N/D")
        strength = team.get("stats", {}).get("strengthScore09", 0)
        print(f"   {strength:5.2f} - {name}")

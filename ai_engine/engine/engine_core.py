...
try: import ai_engine.calculators.calculator_affidabilita_engine as reliability_lib
...
if __name__ == "__main__":
    ...
    print("1-5: Algoritmi Singoli | 6: MONTE CARLO (500 match) | 7: BENCHMARK ALGO_C")

    try:
        raw_input = input("Scegli Opzione (1-7): ").strip()
        mode = int(raw_input)
    except ValueError:
        mode = 5
    ...
    # Carica bulk_cache identico alla produzione
    print(f"\n📦 Caricamento bulk_cache per {home} vs {away} ({league})...")
    try:
        bulk_cache = bulk_manager.get_all_data_bulk(home, away, league)
        print("✅ Bulk cache caricato!")
    except Exception as e:
        print(f"❌ Errore bulk_manager: {e}")
        import traceback
        traceback.print_exc()
        bulk_cache = None

    if mode == 6 or mode == 7:
        algo_mode = 7 if mode == 7 else 5
        label = "BENCHMARK ALGO_C" if mode == 7 else "MONTE CARLO"
        print(f"\n🎲 AVVIO {label}: {home} vs {away}")
        print("⏳ Pre-caricamento dati...")

        try:
            static_data = preload_match_data(home, away, bulk_cache=bulk_cache)
        except Exception as e:
        ...
        print(f"🚀 {simulations} simulazioni (algo={algo_mode})...")
        for i in range(simulations):
            try:
                s_h, s_a, _, _ = predict_match(home, away, mode=algo_mode, preloaded_data=static_data, bulk_cache=bulk_cache)
                if s_h is None:
                    continue
        ...
        if results_h:
            ...
            print("\n" + "=" * 50)
            print(f"📊 {label} FINITO ({len(results_h)} iterazioni valide)")
            print("=" * 50)
        ...
        try:
            static_data = preload_match_data(home, away, bulk_cache=bulk_cache)
            predict_match(home, away, mode=mode, preloaded_data=static_data, bulk_cache=bulk_cache)
        except Exception as e:
            print(f"❌ Errore: {e}")
            import traceback
            traceback.print_exc()
